...
def resize_CAM(classmap_val):
    # resize class activation map to (IMAGE_SIZE, IMAGE_SIZE)
    return skimage.transform.resize(classmap_val, (IMAGE_SIZE, IMAGE_SIZE))
            ...
            for step in xrange(1, len(eval_set_queue)+1):
                ...
                if pos_prob >= 0.5:
                    ...
                    if label == [0]: # FP
                        ...
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'FP', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_original.png'), img)
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'FP', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_CAM.png'), resize_CAM(CAM_val))

                    else: # TP
                        stats[region_type][0] += 1
                        # save original image and CAM.
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'TP', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_original.png'),img)
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'TP', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_CAM.png'), resize_CAM(CAM_val))
                    ...
                    if label == [1]:  # FN
                        ...
                        CAM_val = rescale_CAM(CAM_val)
                        # save original image and CAM.
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'FN', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_original.png'),img)
                        skimage.io.imsave(os.path.join(RESULT_DIR, 'FN', str(region_index) + '_' + str(img_index) + '_' + format(pos_prob, '.2f') + '_CAM.png'), resize_CAM(CAM_val))
...
if __name__ == '__main__':
    if not os.path.exists(RESULT_DIR):
        ...
        os.mkdir(os.path.join(RESULT_DIR, 'FN'))
    test()
